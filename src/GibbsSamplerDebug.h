#ifndef __GSDEGUB_HPP__
#define __GSDEGUB_HPP__
...
class FGMsamplerDegub : public SamplerTraits
{
	...
	FGMsamplerDegub( MatCol const & _data, Parameters const & _params, Hyperparameters const & _hy_params, 
			   		 ...
			   		 MatCol _Beta, VecCol _mu, double _taueps, MatRow _K, unsigned int _seed = 0):
				...
				TBeta(_Beta), Tmu(_mu), Ttau(_taueps), TK(_K)
	{
	 	this->check();
	 	if(seed == 0)
...
FGMsamplerDegub<GraphStructure, T, RetGraph>::run()
{
	...
	sample::rgamma  rGamma; //Watch out, it follows shape-scale parametrization. (i.e mean = shape*scale)
