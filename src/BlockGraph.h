...
template<class T = unsigned int>
class BlockGraphAdj : public BlockGraphBase<T>{
public:
  ...
  BlockGraphAdj(GroupsPtr const & _Gr): BlockGraphBase<T>(_Gr){};
  BlockGraphAdj(typename BlockGraphBase<T>::Adj const & _A, GroupsPtr const & _Gr):BlockGraphBase<T>(_Gr), data(_A){
...
template<class T>
void BlockGraphAdj<T>::set_graph(typename BlockGraphBase<T>::Adj&& A){
...
template<class T>
void BlockGraphAdj<T>::set_graph(typename BlockGraphBase<T>::Adj const & A){
  unsigned int M{this->ptr_groups->get_n_groups()};
  if( M*(M-1)/2 + M - this->n_singleton != A.size() )
    throw std::runtime_error("The number of groups is not coherent with the size of the adjacency matrix");
...
template<class T>
void BlockGraphAdj<T>::add_link(typename BlockGraphBase<T>::IdxType const & i, typename BlockGraphBase<T>::IdxType const & j){
...
template<class T>
void BlockGraphAdj<T>::remove_link(typename BlockGraphBase<T>::IdxType const & i, typename BlockGraphBase<T>::IdxType const & j){
...
template<class T>
T BlockGraphAdj<T>::operator()(typename BlockGraphBase<T>::IdxType const & i, typename BlockGraphBase<T>::IdxType const & j)const{
  if(i == j){
    std::vector<unsigned int> singleton = this->ptr_groups->get_pos_singleton();
    auto it = std::find(singleton.cbegin(), singleton.cend(), i);
    if(it == singleton.cend() )
...
//template<class T>
//T& BlockGraphAdj<T>::operator()(typename BlockGraphBase<T>::IdxType const & i, typename BlockGraphBase<T>::IdxType const & j){
//
  //if(i == j){
    //std::vector<unsigned int> singleton = this->ptr_groups->get_pos_singleton();
    //auto it = std::find(singleton.cbegin(), singleton.cend(), i);
...
template<class T=unsigned int>
class BlockGraph : public BlockGraphBase<T>{
public:
  using value_type    = typename BlockGraphBase<T>::value_type;
  using Adj 			    = typename BlockGraphBase<T>::Adj;
  using IdxType		    = typename BlockGraphBase<T>::IdxType;
  ...
  using Neighbourhood = typename BlockGraphBase<T>::Neighbourhood;
  //Constructors
  BlockGraph(Adj const & _A, GroupsPtr const & _Gr):BlockGraphBase<T>(_Gr){
    unsigned int M{this->ptr_groups->get_n_groups()};
