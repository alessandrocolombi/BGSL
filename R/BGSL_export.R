...
FLM_sampling = function( p, data, niter = 100000, burnin = niter/2, thin = 1, diagonal_graph = T, G = NULL ,
                         Param = NULL, HyParam = NULL, Init = NULL, progressBar = TRUE, seed = 0 )
{
  n = dim(data)[2]
  ...
  return (FLM_sampling_c( data, niter, burnin, thin, Param$BaseMat, G, 
                          ...
                          diagonal_graph, Param$threshold, progressBar
                        )
         )
}
...
LM_init = function(p ,n, empty = TRUE, K0 = NULL, Beta0 = NULL, mu0 = NULL, tau_eps0 = NULL, tauK0 = NULL, seed = 0)
{
  ...
  if(empty){
    ...
    if(is.null(K0)){
      I = diag(p)
      K0 = rGwish(I, 3, I, groups = groups, threshold_conv = 1e-16, seed = seed)$Matrix
    }
    if(is.null(tau_eps0)){
      tau_eps0 = rgamma(n = 1, shape = 50, rate = 2)
    }
    ...
    if(is.null(tauK0)){
      tauK0 = rep(0,p)
      for(i in 1:p){
        tauK0[i] = rgamma(n = 1, shape = 50, rate = 2)
      }
    }
    if(is.null(Beta0)){
      Beta0 = matrix(0,nrow = p, ncol = n)
